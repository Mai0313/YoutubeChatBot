...
- **訊息過濾**: 過濾和處理特定類型的訊息
- **用戶識別**: 追蹤訊息作者及其詳細資訊
- **免授權唯讀模式（新）**: 使用 ChatDownloader 在無 API 金鑰的情況下載取聊天室訊息
...
for line in stream.iter_chat_messages_no_auth(inactivity_timeout=1.0):
    print(line)  # 例如 "Alice: Hello!"
```

注意：

- 此模式使用 `chat-downloader`，不需要 API 金鑰或 OAuth。
- 僅支援讀取訊息，不支援發送訊息。
- 穩定性可能受 YouTube 網站變動影響。
...
- **聊天訊息（API 模式）**: 每則訊息約 1 個單位
- **發送訊息（API 模式）**: 每則訊息約 50 個單位
- **免授權模式**: 不消耗 API 配額，但僅能讀取，依賴網站擷取
